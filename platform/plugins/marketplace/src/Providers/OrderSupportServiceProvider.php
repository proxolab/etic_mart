...
class OrderSupportServiceProvider extends ServiceProvider
{
    ...
    public function processPostCheckoutOrder($products, $request, $token, $sessionCheckoutData, $response)
    {
        ...
        $totalAmount = $orders->pluck('amount')->sum(); // Calculator in here
    ...
    public function handleCheckoutOrderByStore(
        ...
        if (($promotionDiscountAmount + $couponDiscountAmount - $shippingAmount) > $rawTotal) {
            $orderAmount = 0;
        } else {
    ...
    public function processPaymentMethodPostCheckout($request, $totalAmount)
    {
        ...
        switch ($request->input('payment_method')) {
            case PaymentMethodEnum::STRIPE:
                $result = $this->app->make(StripePaymentService::class)->execute($request);
                if ($this->app->make(StripePaymentService::class)->getErrorMessage()) {
                    $paymentData['error'] = true;
                    $paymentData['message'] = $this->app->make(StripePaymentService::class)->getErrorMessage();
                }
            ...
            case PaymentMethodEnum::PAYPAL:
                $checkoutUrl = $this->app->make(PayPalPaymentService::class)->execute($request);
                if ($checkoutUrl) {
                    $paymentData['checkoutUrl'] = $checkoutUrl;
                } else {
                    $paymentData['error'] = true;
                    $paymentData['message'] = $this->app->make(PayPalPaymentService::class)->getErrorMessage();
                }
    ...
    public function processShippingDiscountOrderData($products, $token, $sessionCheckoutData, $request)
    {
        ...
        foreach ($groupedProducts as $storeId => $productsByStore) {
            ...
            $marketplaceData[$storeId] = compact('shipping',
        ...
        $shippingAmount = $marketplaceData->pluck('shippingAmount')->sum();
        $promotionDiscountAmount = $marketplaceData->pluck('promotionDiscountAmount')->sum();
        $couponDiscountAmount = $marketplaceData->pluck('couponDiscountAmount')->sum();
        ...
        if (session()->has('applied_coupon_code')) {
            ...
            foreach ($mpSessionData as $storeId => $sessionStoreData) {
                $discount = $this->app->make(HandleApplyCouponService::class)
                    ->getCouponData(session('applied_coupon_code'),
                        $sessionStoreData);
                if (!$discount) {
                    $discounts->push($discount);
                    $prefix = "marketplace.$storeId.";
    ...
    public function processCheckoutRulesRequest($rules)
    {
        ...
        $storeIds = Cart::instance('cart')->products()->pluck('original_product')->pluck('store_id');
    ...
    {
        $storeIds = Cart::instance('cart')->products()->pluck('original_product')->pluck('store_id');
        $stores = $this->app->make(StoreInterface::class)->allBy([['id', 'IN', $storeIds]]);
        foreach ($stores as $store) {
            $messages["shipping_method.$store->id.required"] = trans('plugins/marketplace::order.shipping_method_required',
                ['name' => $store->name]);
            $messages["shipping_method.$store->id.in"] = trans('plugins/marketplace::order.shipping_method_in',
                ['name' => $store->name]);
            $messages["shipping_option.$store->id.required"] = trans('plugins/marketplace::order.shipping_option_required',
                ['name' => $store->name]);
        }
    ...
    public function afterOrderStatusCompleted(OrderModel $order)
    {
        ...
        $order->loadMissing(['currency', 'store', 'store.customer']);
        if ($order->store && $order->store->customer) {
            ...
            if ($vendorInfo->id) {
                ...
                if ($order->currency_id != get_application_currency_id()) {
                    if ($order->currency) {
                        $exchangeRate = get_current_exchange_rate($order->currency);
                        $amountByCurrency = $amountByCurrency / $exchangeRate;
                        $revenueAmount = $revenueAmount / $exchangeRate;
                    }
                }
